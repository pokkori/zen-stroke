  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--washi)", fontFamily: "Noto Serif JP, serif" }}>
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-4 gap-4">
        ...
        {alreadyPlayed && !score && !showResult && (<div style={{ width: "100%", maxWidth: "384px", borderRadius: "16px", padding: "20px", textAlign: "center", background: "#fff", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: "8px" }}><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div><div style={{ fontWeight: 700 }}>今日はプレイ済みです</div><div style={{ fontSize: "0.85rem", color: "#888" }}>明日また新しい漢字が出ます</div>{streakData && streakData.streak >= 1 && <div style={{ marginTop: "12px", padding: "8px", background: "rgba(192,57,43,0.06)", borderRadius: "12px" }}>{streakData.streak}日連続プレイ中！</div>}</div>)}
      </main>
