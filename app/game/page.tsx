      ...
      <main className="flex-1 flex flex-col items-center px-4 py-4 gap-4">
        ...
        {!showResult && (
          <div className="flex items-center gap-3">
            <button onClick={undo} disabled={!hasDrawn} style={{ background: hasDrawn ? "rgba(26,26,26,0.08)" : "rgba(26,26,26,0.04)", color: hasDrawn ? "var(--sumi)" : "#aaa", border: "1px solid rgba(26,26,26,0.12)", cursor: hasDrawn ? "pointer" : "not-allowed", padding: "8px 16px", borderRadius: "8px", fontSize: "0.85rem" }}>↩ 一手戻す</button>
            <button onClick={handleClear} disabled={!hasDrawn} style={{ background: hasDrawn ? "rgba(192,57,43,0.08)" : "rgba(26,26,26,0.04)", color: hasDrawn ? "var(--vermillion)" : "#aaa", border: "1px solid rgba(192,57,43,0.2)", cursor: hasDrawn ? "pointer" : "not-allowed", padding: "8px 16px", borderRadius: "8px", fontSize: "0.85rem" }}>✕ クリア</button>
            <button onClick={handleSubmit} disabled={!hasDrawn || isSubmitting || alreadyPlayed} style={{ background: (hasDrawn && !alreadyPlayed) ? "var(--vermillion)" : "rgba(26,26,26,0.15)", color: (hasDrawn && !alreadyPlayed) ? "#fff" : "#aaa", cursor: (hasDrawn && !alreadyPlayed) ? "pointer" : "not-allowed", padding: "8px 24px", borderRadius: "10px", fontSize: "0.9rem", fontWeight: 700, border: "none" }}>{isSubmitting ? "採点中…" : (alreadyPlayed && !score) ? "今日はプレイ済み" : "提出する"}</button>
          </div>
        )}
        ...
        {showResult && score && (
          <div style={{ width: "100%", maxWidth: "384px", borderRadius: "16px", padding: "20px", background: "#fff", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", animation: "scoreReveal 0.6s ease forwards" }}>
            ...
            <button onClick={handleShare} style={{ width: "100%", padding: "12px", borderRadius: "12px", fontWeight: 700, color: "#fff", background: "#000", border: "none", cursor: "pointer" }}>𝕏 結果をシェアする</button>
